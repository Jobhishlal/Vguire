...
export const buyNowCartView = async (req, res) => {
  try {
    ...
    if (
      ...
      typeof req.session.appliedCoupon === "string"
    ) {
...
export const cartproduct = async (req, res) => {
  ...
  try {
    ...
    const totalAmount = cart.items.reduce(
      (total, item) => total + item.productId.Offerprice * item.quantity,
      0
    );
...
export const placeorder = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(addressId)) {
      return res
        .status(400)
        .send("Invalid address selected. Please choose a valid address.");
    }
    ...
    if (updatedItems.length === 0) {
      ...
      return res
        .status(400)
        .send("All items are out of stock. Please update your cart.");
    }
    ...
    if (req.session.appliedCoupon) {
      const couponCode =
        typeof req.session.appliedCoupon === "object"
    ...
    if (paymentMethod === "COD" && totalAmount > 1000) {
      return res.status(400).json({
        success: false,
        message:
          "Cash on Delivery is not available for orders above ₹1000. Please choose another payment method.",
      });
    }
...
export const addaddress = async (req, res) => {
  try {
    ...
    if (!addressType || !["Home", "Work", "Other"].includes(addressType))
      errors.push("Invalid address type selected.");
    ...
    return res.json({
      success: true,
      redirectUrl: redirectUrl || "/user/checkout/buy-now",
    });
  } catch (error) {
...
export const editaddress = async (req, res) => {
  try {
    ...
    if (!updatedAddress) {
      return res
        .status(404)
        .json({ success: false, errors: ["Address not found"] });
    }

    if (productId && quantity && size) {
      return res.json({
        success: true,
        redirectUrl: `/user/checkout/single?productId=${productId}&quantity=${quantity}&size=${size}`,
      });
    } else {
      return res.json({
        success: true,
        redirectUrl: redirectUrl || "/user/checkout",
      });
    }
  } catch (error) {
...
const generateInvoice = (res, order) => {
  ...
  // Address Details
  doc
    .text(`Address: ${order.addressId?.fullName || "N/A"}`)
  ...
  doc
    ...
    .fillColor("#0000FF")
    .font("Helvetica-Bold")
    .text("Product", columnPositions[0], tableTop, {
      width: columnWidths[0],
      align: "left",
    })
    .text("Payment", columnPositions[1], tableTop, {
      width: columnWidths[1],
      align: "left",
    })
    .text("Quantity", columnPositions[2], tableTop, {
      width: columnWidths[2],
      align: "right",
    })
    .text("Size", columnPositions[3], tableTop, {
      width: columnWidths[3],
      align: "right",
    })
    .text("Amount", columnPositions[4], tableTop, {
  ...
  order.items.forEach((item) => {
    ...
    if (typeof totalPrice === "number") {
      ...
      doc
        ...
        .fillColor("#000000")
        .font("Helvetica")
        .text(`${item.productId?.name || "N/A"}`, columnPositions[0], yOffset, {
          width: columnWidths[0],
          align: "left",
        })
        .text(`${order.paymentMethod || "N/A"}`, columnPositions[1], yOffset, {
          width: columnWidths[1],
          align: "left",
        })
        .text(`${item.quantity}`, columnPositions[2], yOffset, {
          width: columnWidths[2],
          align: "right",
        })
        .text(`${item.size || "N/A"}`, columnPositions[3], yOffset, {
          width: columnWidths[3],
          align: "right",
        })
        .text(`${totalPrice.toFixed(2)}`, columnPositions[4], yOffset, {
          width: columnWidths[4],
          align: "right",
        });
      ...
      doc
        .moveTo(50, yOffset + 10)
        .lineTo(540, yOffset + 10)
        .stroke();
    } else {
  ...
  if (typeof subtotal === "number") {
    ...
    doc
      ...
      .fillColor("#000000")
      .font("Helvetica-Bold")
      .text(`Subtotal: ${subtotal.toFixed(2)}`, 50, currentY + 10, {
        align: "left",
      })
      .text(
        `Total Product Discount: ${(order.totalProductDiscount || 0).toFixed(2)}`,
        { align: "left" }
      )
    ...
    if (order.refundedAmount && order.refundedAmount > 0) {
      doc
        .fillColor("#008000")
        .font("Helvetica-Bold")
        .text(`Refund Amount: ${order.refundedAmount.toFixed(2)}`, {
          align: "left",
        });
    }
    ...
    doc
      .fillColor("#FF0000")
      .text(`Final Total Amount: ${order.totalAmount.toFixed(2)}`, {
        align: "left",
        bold: true,
      });
  } else {
...
export const orderview = async (req, res) => {
  ...
  try {
    ...
    validItems.forEach((item) => {
      ...
      let discountPerItem =
        productPrice > offerPrice ? productPrice - offerPrice : 0;
...
export const ordercancel = async (req, res) => {
  try {
    ...
    if (["Shipped", "Delivered"].includes(order.items[productIndex].status)) {
      return res.status(400).json({
        message:
          "This product cannot be cancelled as it is already shipped or delivered",
      });
    }
    ...
    if (order.appliedCoupon) {
      ...
      const totalDiscount =
        originalOrderTotal - order.totalAmount - (order.refundedAmount || 0);

      proportionalDiscount =
        (cancelledItemTotal / originalOrderTotal) * totalDiscount;

      refundAmount = cancelledItemTotal - proportionalDiscount;
    }

    // Update order item with refunded amount and status
    ...
    // Adjust total amount and refunded amount in order
    const previousTotal = order.totalAmount;
    order.totalAmount = previousTotal - refundAmount;
    if (order.items.quantity < 5) {
      order.refundedAmount = order.totalAmount / 2;
    } else if (order.totalAmount < 0) order.totalAmount = 0;
    ...
    if (order.items.every((item) => item.status === "Cancelled")) {
      ...
      order.paymentStatus =
        order.paymentMethod === "UPI" && order.paymentStatus === "Pending"
          ? "Pending" // Keep pending if payment is not yet completed
          : "Refunded"; // Otherwise mark as refunded
    }

    await order.save();

    // Handle refund to wallet if payment was by UPI and NOT pending
    let updatedWalletBalance = null;
    let refundNote = null;

    if (order.paymentMethod === "UPI") {
      if (order.paymentStatus !== "Pending") {
        // Payment completed, refund to wallet
        const currentUser = await User.findById(req.user._id);
        if (currentUser) {
          currentUser.walletBalance =
            (currentUser.walletBalance || 0) + refundAmount;
        ...
        refundNote =
          "Payment is pending. Refund will be processed once payment is confirmed.";
      }
    }
    ...
    res.json({
      message:
        order.status === "Cancelled"
          ? "Full order cancelled successfully"
...
export const ratingadd = async (req, res) => {
  try {
    ...
    const totalRatings = ratedOrders.flatMap((order) =>
      order.items.filter(
        (i) => i.productId.toString() === productId && i.rating
      )
    );
    const averageRating =
      totalRatings.reduce((sum, i) => sum + i.rating, 0) / totalRatings.length;
  ...
  try {
    ...
    if (updatedItems.totalAmount <= 0) {
      return res
        ...
        .json({
          success: false,
          message: "total Amount 0 is not possible to share",
        });
    }
...
export const couponapplied = async (req, res) => {
  try {
    ...
    if (new Date() > new Date(coupon.expirationDate)) {
      return res
        .status(400)
        .json({ success: false, type: "expired", message: "Coupon expired" });
    }
    ...
    // Set in session for later use (order placement)
...
export const returnsingle = async (req, res) => {
  try {
    ...
    if (productItem.returnStatus === "Requested") {
      return res.status(400).json({
        message:
          "A return request for this product is already pending approval.",
      });
    }

    if (
      productItem.returnStatus === "Approved" ||
      productItem.returnStatus === "Returned"
    ) {
      return res
        .status(400)
        .json({ message: "This product has already been returned." });
    }
...
export const returnentireorder = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message:
        "Return request submitted for entire order. Admin approval pending.",
...
export const placeorderwallet = async (req, res) => {
  try {
    ...
    if (!User) {
      return res
        .status(500)
        .json({ success: false, message: "Userr not found" });
    }
    if (!mongoose.Types.ObjectId.isValid(addressId)) {
      return res
        .statu(500)
        .json({ success: false, message: "address is not get" });
    }
    if (!items || !Array.isArray(items) || items.length === 0) {
      return res
        .status(5000)
        .json({ success: false, message: "itsm not get properly" });
    }
    const cart = await Cart.findOne({ userId }).populate("items.productId");
    const productIds = items.map((item) => item.productId);
    const products = await Product.find({ _id: { $in: productIds } });
    ...
    if (user.walletBalance < totalAmount) {
      return res.status(400).json({
        success: false,
        message:
          "Your wallet balance is low. Please choose another payment method.",
      });
    }
